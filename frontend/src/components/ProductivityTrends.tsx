import {
  ...
  const ProductivityTrends = ({ productivity }: Props) => {
    const [range, setRange] = useState<"lastSeven" | "lastThirty" | "lastSixtyFive">("lastSeven");
      ...
      <div className="bg-white p-6 rounded-lg shadow-md dark:bg-zinc-800 mb-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex space-x-2">
            {(["lastSeven", "lastThirty", "lastSixtyFive"] as const).map((key) => (
                <button
                key={key}
        ...
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={formattedData}>
            ...
            <Tooltip />
            <Line type="monotone" dataKey="productivity" stroke="#60A5FA" strokeWidth={3} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    );
